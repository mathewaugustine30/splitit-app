...
import { ActivityFilters, User, Group } from '../types';
...
const ActivityFiltersComponent: React.FC<ActivityFiltersProps> = ({ filters, onFiltersChange, users, groups }) => {
  const [isOpen, setIsOpen] = useState(false);

  const handleGroupChange = (groupId: string) => {
    const newGroups = filters.groups.includes(groupId)
      ? filters.groups.filter(c => c !== groupId)
      : [...filters.groups, groupId];
    onFiltersChange({ ...filters, groups: newGroups });
  };

  const handleParticipantChange = (participantId: string) => {
    const newParticipants = filters.participants.includes(participantId)
      ? filters.participants.filter(p => p !== participantId)
      : [...filters.participants, participantId];
    onFiltersChange({ ...filters, participants: newParticipants });
  };
  ...
  const activeFilterCount =
    ...
    (filters.participants.length > 0 ? 1 : 0) +
      ...
      {isOpen && (
        <div className="mt-4 pt-4 border-t">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {/* Groups */}
            <div>
              <h4 className="font-semibold mb-2 text-gray-600">Groups</h4>
            ...
            {/* Participants */}
            <div>
              <h4 className="font-semibold mb-2 text-gray-600">People</h4>
              <div className="space-y-1 max-h-40 overflow-y-auto">
                {users.map(user => (
                  <label key={user.id} className="flex items-center space-x-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={filters.participants.includes(user.id)}
