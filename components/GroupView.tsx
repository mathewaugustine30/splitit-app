...
import { PlusIcon, EditIcon, PaperclipIcon } from './icons';
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        <div className="flex flex-col sm:flex-row justify-between sm:items-start">
            ...
            <button 
                ...
                className="flex items-center bg-secondary text-white font-semibold py-2 px-4 rounded-md text-sm hover:bg-blue-600 transition-colors self-start sm:self-auto"
            >
      ...
      <div className="bg-white p-6 rounded-lg shadow mb-8">
        ...
        <ul className="space-y-3">
          {groupMembers.filter(m => m.id !== currentUserId).map(member => {
            ...
            return (
              <li key={member.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between">
                ...
                {balance && (
                    <button onClick={() => onSettleUp(member.id, balance.amount)} className="bg-secondary text-white font-semibold py-1 px-3 rounded-md text-sm hover:bg-blue-600 transition-colors self-end sm:self-auto">
                        Settle Up
                    </button>
                )}
              </li>
        ...
        <ul className="space-y-4">
          {filteredGroupExpenses.map(expense => {
           ...
           {filteredGroupExpenses.length === 0 && (
              <p className="text-gray-500 text-center py-4">No expenses in this group match the current filters.</p>
           )}
        </ul>
      </div>
    </div>
